  ...
  public:
    mdns_registration_t():
      ...
      if (service(true, existing_instance)) {
        BOOST_LOG(error) << "Unable to register Nimbus mDNS service"sv;
        return;
      }

      BOOST_LOG(info) << "Registered Nimbus mDNS service"sv;
    }

    ~mdns_registration_t() override {
      if (existing_instance) {
        if (service(false, existing_instance)) {
          BOOST_LOG(error) << "Unable to unregister Nimbus mDNS service"sv;
          return;
        }

        BOOST_LOG(info) << "Unregistered Nimbus mDNS service"sv;
      }
    }
