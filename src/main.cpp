...
#pragma GCC diagnostic pop
...
#ifdef _WIN32
    BOOST_LOG(fatal) << "To relaunch Nimbus successfully, use the shortcut in the Start Menu. Do not run sunshine.exe manually."sv;
  ...
  if (config::sunshine.port == 47989 && is_gamestream_enabled()) {
    BOOST_LOG(fatal) << "GameStream is still enabled in GeForce Experience! This *will* cause streaming problems with Nimbus!"sv;
    BOOST_LOG(fatal) << "Disable GameStream on the SHIELD tab in GeForce Experience or change the Port setting on the Advanced tab in the Nimbus Web UI."sv;
  }
#endif
