...
async function refreshFrameGenHealth(options: FrameGenHealthOptions = {}): Promise<void> {
  ...
  const run = async () => {
    ...
    try {
      ...
      if (rtssResult.status === 'fulfilled') {
        ...
        if (ok) {
          ...
          if (rtssInstalled && rtssHooks) {
            rtssStatus = 'pass';
            rtssMessage = 'RTSS hooks detected. Nimbus can control the frame limiter.';
          } else if (rtssInstalled) {
            rtssStatus = 'warn';
            rtssMessage =
              'RTSS is installed but hooks were not detected. Launch RTSS and ensure the managed RTSS profile is active.';
          } else {
            rtssStatus = 'fail';
      ...
      let displayLabel = usingVirtual ? 'Nimbus Virtual Screen' : 'Active display';
      ...
      if (!usingVirtual) {
            ...
            if (target) {
              ...
              if (evaluationHz === null) {
                ...
                if (only144Fails) {
                  ...
                  displayMessage = `Current refresh is ${Math.round(baseHz)} Hz. Streams up to 120 FPS are covered. Only 144 FPS streams require the Nimbus virtual screen or a higher-refresh display.`;
                  if (!hasActive && highestSupported !== null) {
                    displayMessage = `Display supports up to ${Math.round(highestSupported)} Hz. Streams up to 120 FPS are covered. Only 144 FPS streams require the Nimbus virtual screen or a higher-refresh display.`;
                  } else if (deltaSupported && highestSupported !== null) {
                    displayMessage += ` Nimbus can switch to ${Math.round(highestSupported)} Hz when a stream starts if Display Device Step 1 keeps that monitor active.`;
                  }
                } else if (!hasActive && highestSupported !== null) {
                  displayMessage = `Display supports up to ${Math.round(highestSupported)} Hz. Nimbus can double 120 FPS streams.`;
                } else if (deltaSupported && highestSupported !== null) {
                  displayMessage = `Current refresh is ${Math.round(activeRefresh ?? evaluationHz)} Hz. Nimbus can switch to ${Math.round(highestSupported)} Hz during streams to keep frame generation smooth.`;
                } else {
                ...
                displayStatus = 'warn';
                if (!hasActive && highestSupported !== null) {
                  displayMessage = `Display supports up to ${Math.round(evaluationHz)} Hz. Configure Display Device Step 1 to enforce the higher refresh or use the display override below to switch to the Nimbus virtual display.`;
                } else if (hasActive) {
                  if (highestFailUnder144 !== null) {
                    displayMessage = `Current refresh is ${Math.round(activeRefresh ?? evaluationHz)} Hz. Streams targeting up to ${highestFailUnder144} FPS need the Nimbus virtual screen or a higher-refresh display.`;
                  } else {
                    displayMessage = `Current refresh is ${Math.round(activeRefresh ?? evaluationHz)} Hz. 120 FPS frame generation may stutter without a higher refresh display. Use the display override below to switch to the Nimbus virtual display or move the stream to a higher-refresh monitor.`;
                  }
                  if (deltaSupported && highestSupported !== null) {
                    displayMessage += ` Nimbus can switch up to ${Math.round(highestSupported)} Hz if Display Device Step 1 keeps only that monitor active.`;
                  }
                } else {
                  displayMessage =
                    'Unable to read the current refresh rate, but the display may not reach the required 240 Hz. Use the display override below to switch to the Nimbus virtual display or move the stream to a higher-refresh monitor.';
                }
              } else {
                displayStatus = 'fail';
                if (!hasActive && highestSupported !== null) {
                  displayMessage = `Display tops out at ${Math.round(evaluationHz)} Hz. Use the display override below to switch to the Nimbus virtual display or switch to a 240 Hz display for frame generation.`;
                } else if (hasActive) {
                  const mention = highestFailUnder144 ?? 120;
                  displayMessage = `Current refresh is ${Math.round(activeRefresh ?? evaluationHz)} Hz. Streams targeting up to ${mention} FPS need the Nimbus virtual screen or a higher-refresh display.`;
                  if (deltaSupported && highestSupported !== null) {
                    displayMessage += ` Nimbus can switch up to ${Math.round(highestSupported)} Hz if configured in Display Device Step 1.`;
                  }
                } else {
                  displayMessage =
                    'Display refresh information was unavailable. Use the display override below to switch to the Nimbus virtual display or switch to a 240 Hz display for frame generation.';
                }
              }
            } else {
              displayStatus = 'unknown';
              displayMessage =
                'No display devices were returned by the Nimbus helper. Frame generation may not be able to enforce refresh changes.';
        ...
        displayStatus = 'pass';
        displayMessage =
          'Nimbus virtual screen guarantees a high refresh surface for frame generation.';
      }
      ...
      if (highestFailUnder144 !== null) {
        health.suggestion = {
          message: `Use the display override above to switch to the Nimbus virtual display or configure Display Device Step 1 to target the virtual display so ${highestFailUnder144} FPS streams stay smooth.`,
          emphasis: 'warning',
        };
...
function warnIfHealthIssues(reason: FrameGenHealthReason) {
  ...
  if (!skipDisplayWarnings.value && !health.display.virtualActive) {
    ...
    if (requiresHigh) {
      message.warning(
        'Use the display override to switch to the Nimbus virtual display or adjust Display Device Step 1 to keep only the high-refresh monitor active.',
        { duration: 8000 },
      );
    }
  }
...
watch(
  ...
  async (enabled) => {
    ...
    message?.info(
      "Frame Generation Capture Fix requires Windows Graphics Capture (WGC), RTSS, and a display capable of 240 Hz or higher. Nimbus's virtual screen or any display that satisfies the doubled refresh requirement will work.",
      { duration: 8000 },
    );
    if (!skipDisplayWarnings.value) {
      if (!ddConfigOption.value || ddConfigOption.value === 'disabled') {
        message?.warning(
          'Configure Step 1 for Nimbus\'s virtual screen or enable Display Device and set it to "Deactivate all other displays" so the doubled refresh requirement is met during the stream.',
          { duration: 8000 },
        );
      } else if (ddConfigOption.value !== 'ensure_only_display') {
        message?.warning(
          'Set Step 1 to use Nimbus\'s virtual screen or adjust Display Device to "Deactivate all other displays" so only the high-refresh monitor stays active.',
          { duration: 8000 },
        );
      }
    }
...
// Cover preview logic removed; Nimbus no longer fetches or proxies images
async function save() {
  saving.value = true;
  try {
